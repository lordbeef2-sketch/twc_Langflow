...
import FlowShareModal from "@/modals/flowShareModal";
import useAlertStore from "@/stores/alertStore";
import useAuthStore from "@/stores/authStore";
import useFlowStore from "@/stores/flowStore";
import useFlowsManagerStore from "@/stores/flowsManagerStore";
import { isFlowOwner } from "@/utils/flowAccess";
...
export default function PublishDropdown({
  ...
  const [openFlowShareModal, setOpenFlowShareModal] = useState(false);
  const canManageSharing = isFlowOwner(currentFlow);
  ...
  return (
    <>
      <DropdownMenu>
        ...
        >
          ...
          {canManageSharing && (
            <DropdownMenuItem
              className="deploy-dropdown-item group"
              onClick={() => setOpenFlowShareModal(true)}
            >
              <IconComponent name="UserRoundPlus" className={`icon-size mr-2`} />
            ...
            >
              <div className="flex w-full items-center justify-between">
                ...
                <Switch
                  ...
                  className="scale-[85%]"
                  checked={isPublished}
                  disabled={!hasIO || !canManageSharing}
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
      ...
      <FlowShareModal
        open={openFlowShareModal}
        setOpen={setOpenFlowShareModal}
        flow={currentFlow}
      />
    </>
  );
