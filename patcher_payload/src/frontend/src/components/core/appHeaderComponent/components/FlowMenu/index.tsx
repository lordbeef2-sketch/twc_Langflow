...
import { canEditFlow, getFlowViewerFolderId } from "@/utils/flowAccess";
...
export const MenuBar = memo((): JSX.Element => {
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      currentFlowFolderId: getFlowViewerFolderId(state.currentFlow),
  ...
  const editableFlow = canEditFlow(currentFlow);

  const handleSave = () => {
    if (!onFlowPage || !editableFlow) return;
    saveFlow().then(() => {
            ...
            >
              <div>
                <Button
                  ...
                  size="iconMd"
                  disabled={
                    !editableFlow || !changesNotSaved || isBuilding || saveLoading
                  }
