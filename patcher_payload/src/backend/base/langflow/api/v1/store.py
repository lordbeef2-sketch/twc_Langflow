...
def is_langpatcher_local_only() -> bool:
    value = os.getenv("LANGPATCHER_LOCAL_ONLY", "")
    return value.strip().lower() in {"1", "true", "yes", "on"}


def local_only_store_disabled() -> HTTPException:
    return HTTPException(
        status_code=404,
        detail="LangPatcher local-only mode is enabled; the Langflow Store is disabled.",
    )


def get_user_store_api_key(user: CurrentActiveUser):
    if is_langpatcher_local_only():
        raise local_only_store_disabled()
...
def get_optional_user_store_api_key(user: CurrentActiveUser):
    if is_langpatcher_local_only() or not user.store_api_key:
        return None
    try:
        return auth_utils.decrypt_api_key(user.store_api_key)
...
async def check_if_store_is_enabled():
    if is_langpatcher_local_only():
        return {"enabled": False, "local_only": True}
    return {
...
async def check_if_store_has_api_key(
    ...
    if is_langpatcher_local_only():
        return {"has_api_key": False, "is_valid": False, "local_only": True}
...
async def get_components(
    *,
    ...
    if is_langpatcher_local_only():
        return ListComponentResponseModel(count=0, authorized=False, results=[])
