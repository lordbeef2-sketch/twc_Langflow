...
from langflow.api.v1.schemas import ShareableUserRead, UsersResponse
...
@router.get("/shareable", response_model=list[ShareableUserRead])
async def read_shareable_users(
    *,
    current_user: CurrentActiveUser,
    session: DbSession,
    search: str | None = None,
    limit: int = 20,
) -> list[ShareableUserRead]:
    """Retrieve active users that the current user can share flows with."""
    query: SelectOfScalar = select(User).where(User.id != current_user.id).where(User.is_active == True)  # noqa: E712

    if search:
        query = query.where(User.username.ilike(f"%{search}%"))  # type: ignore[attr-defined]

    users = (await session.exec(query.order_by(User.username).limit(limit))).all()
